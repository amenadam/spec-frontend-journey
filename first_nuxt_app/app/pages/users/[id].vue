...
const { data: user, status } = await useFetch(
  `https://spec-frontend-journey.vercel.app/users/${id}`,
);
console.log(user);
</script>
...
<template>
  ...
  >
    ...
    <div class="text-white bg-white/5 px-4 py-6 rounded-2xl">
      <h1 class="font-bold text-xl">User Data</h1>
      <p>
        User name: <strong> {{ user?.data.fullName }}</strong>
      </p>
      <p>
        User type: <strong>{{ user?.data.userType }}</strong>
      </p>
    </div>
